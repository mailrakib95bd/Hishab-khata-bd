...
/* ---------------- personal task reminder/repeat presets ---------------- */
const TASK_REMINDER_PRESETS = [
    { key: "none", label: "কোনোটি না", minutesBefore: null },
    { key: "ontime", label: "নির্ধারিত সময়ে", minutesBefore: 0 },
    { key: "5min", label: "৫ মিনিট আগে", minutesBefore: 5 },
    { key: "10min", label: "১০ মিনিট আগে", minutesBefore: 10 },
    { key: "1hour", label: "১ ঘণ্টা আগে", minutesBefore: 60 },
    { key: "1day", label: "১ দিন আগে", minutesBefore: 1440 },
    { key: "custom", label: "নিজের মতো", minutesBefore: null },
];
const TASK_REPEAT_OPTIONS = [
    { key: "none", label: "নেই" },
    { key: "daily", label: "প্রতিদিন" },
    { key: "weekly", label: "প্রতি সপ্তাহে" },
    { key: "monthly", label: "প্রতি মাসে" },
    { key: "yearly", label: "প্রতি বছরে" },
    { key: "custom", label: "নিজের মতো" },
];
// derives the actual reminderDate/reminderTime fields the EXISTING firing
// loop already checks (unchanged), from the task's own date+time and the
// chosen preset offset — this way no new reminder-firing mechanism is
// needed, the preset is just resolved down to what already works.
function computeTaskReminderFields(taskDate, taskTime, preset, customDate, customTime) {
    if (!preset || preset === "none")
        return { reminderDate: null, reminderTime: null };
    if (preset === "custom")
        return { reminderDate: customDate || null, reminderTime: customTime || null };
    const p = TASK_REMINDER_PRESETS.find((x) => x.key === preset);
    if (!p || p.minutesBefore == null || !taskDate)
        return { reminderDate: null, reminderTime: null };
    const base = new Date(`${taskDate}T${taskTime || "00:00"}:00`);
    if (isNaN(base.getTime()))
        return { reminderDate: null, reminderTime: null };
    base.setMinutes(base.getMinutes() - p.minutesBefore);
    const y = base.getFullYear(), m = String(base.getMonth() + 1).padStart(2, "0"), d = String(base.getDate()).padStart(2, "0");
    const hh = String(base.getHours()).padStart(2, "0"), mm = String(base.getMinutes()).padStart(2, "0");
...
function App() {
    ...
    // per-user completion tracking for admin-published tasks ("এডমিন টাস্ক") —
    // adminTasksCloud is a single shared/global list, so completion can't be
    // stored on the task itself (that would complete it for every user at
    // once). Same pattern as readNoticeIds/readDailyMessageIds above: an
    // array of ids this user has personally checked off, kept in their own
    // data and never written back to the shared adminTasks collection.
    const [completedAdminTaskIds, setCompletedAdminTaskIds] = useState([]);
    useEffect(() => {
    ...
    const applyUserData = useCallback((data) => {
        ...
        setCompletedAdminTaskIds(d.completedAdminTaskIds || []);
    }, []);
    ...
    const persistAll = useCallback((overrides) => persist(userDataKey(loadedIdentityRef.current || "guest"), Object.assign({ transactions,
        ...
        readNoticeIds,
        readDailyMessageIds,
        dismissedDailyMessageIds,
        completedAdminTaskIds }, overrides)), [transactions, budget, tasks, specialDays, debts, expenseCats, incomeCats, categoryBudgets, accountOpening, transfers, profileName, familyMembers, bazarItems, taxes, userAccounts, auditLog, readNoticeIds, readDailyMessageIds, dismissedDailyMessageIds, completedAdminTaskIds, persist]);
    ...
    // browser/PWA notification permission — 'default' | 'granted' | 'denied',
    // or 'unsupported' if the Notification API doesn't exist in this
    // environment at all. Read once on mount; updated after each explicit
    // request (see requestNotificationPermission below)
    ...
    const requestNotificationPermission = useCallback(async () => {
        ...
        try {
            ...
            // some browsers reject if not called from a direct user gesture —
            // the Settings button below always calls this from a click, so
    ...
    const exportBackupJSON = () => JSON.stringify({
        ...
        userAccounts,
        auditLog,
        readNoticeIds, readDailyMessageIds, dismissedDailyMessageIds, completedAdminTaskIds,
        exportedAt: Date.now(),
    }, null, 2);
    ...
    // tracks whether the currently-open top-level panel (Admin/Tax/Accounts/
    // FAQ/Calendar/Notifications/Settings) was reached FROM the 3-dot menu,
    // so closing it (tap-outside, X, or hardware back) can return to the
    // menu — the previous layer — instead of falling all the way through to
    // the Dashboard. Only set when a HamburgerMenu item is tapped; entry
    // points that bypass the menu (Header's calendar/bell/tax icons) leave
    // it false, so those keep closing straight to the Dashboard as before.
    const menuOriginRef = useRef(false);
    const closeMenuPanel = (setter) => {
        setter(false);
        if (menuOriginRef.current) {
            menuOriginRef.current = false;
    ...
    useEffect(() => {
        const onPopState = () => {
            ...
            if (showCalendar) {
                ...
                closeMenuPanel(setShowCalendar);
                return;
            }
            if (editingTx) {
                setEditingTx(null);
            ...
            if (showSettings) {
                closeMenuPanel(setShowSettings);
                return;
            }
            if (showBudget) {
                setShowBudget(false);
    ...
    useBackOverlay(showNotificationCenter, () => closeMenuPanel(setShowNotificationCenter));
    useBackOverlay(showAdminPanel, () => closeMenuPanel(setShowAdminPanel));
    useBackOverlay(showFAQ, () => closeMenuPanel(setShowFAQ));
    useBackOverlay(showTaxPanel, () => closeMenuPanel(setShowTaxPanel));
    ...
    const [faqCloud, setFaqCloud] = useState([]);
    ...
    // the FAQ collection used to be 3 items hard-coded directly in the
    // component. They're preserved here verbatim as seed data — the very
    // first time an admin loads the app and the /faq collection is still
    // empty, these get written once so nothing from the original content
    // is lost. This never runs again once /faq has at least one document.
    const FAQ_SEED = [
        { question: "আমার তথ্য কি নিরাপদ?", answer: "প্রতিটি ব্যবহারকারীর তথ্য আলাদাভাবে সংরক্ষিত হয় এবং শুধুমাত্র সংশ্লিষ্ট ব্যবহারকারী নিজের তথ্য দেখতে পারেন।" },
        { question: "অফলাইনে অ্যাপ কাজ করে কি?", answer: "হ্যাঁ, ইন্টারনেট সংযোগ ছাড়াও অ্যাপ ব্যবহার করা যায়; সংযোগ ফিরলে ডেটা স্বয়ংক্রিয়ভাবে সিঙ্ক হয়।" },
        { question: "ব্যাকআপ কীভাবে নেব?", answer: "সেটিংস থেকে JSON ব্যাকআপ ডাউনলোড করতে পারবেন।" },
    ];
    const fetchAdminContent = useCallback(async () => {
        ...
        try {
            const [n, d, s, t, f] = await Promise.all([
                ...
                window.FB.listCollection("faq"),
            ]);
            setNotices(n);
            setDailyMessages(d);
            setAdminSpecialDaysCloud(s);
            setAdminTasksCloud(t);
            if (f.length === 0 && isAdmin(user)) {
                // one-time seed, admin session only (matches Firestore
                // write rules) — re-fetch afterward so state reflects what
                // was actually written, not the empty pre-seed result
                await window.FB.batchAddTo("faq", FAQ_SEED);
                setFaqCloud(await window.FB.listCollection("faq"));
            }
            else {
                setFaqCloud(f);
            }
    ...
    // toggles THIS user's own completion mark on a shared admin task —
    // never mutates adminTasksCloud itself (that's a single shared
    // document/collection every user reads), only this user's own
    // completedAdminTaskIds list, exactly mirroring how readNoticeIds
    // tracks per-user read state on shared notices above.
    const toggleAdminTaskComplete = (id) => {
        const next = completedAdminTaskIds.includes(id)
            ? completedAdminTaskIds.filter((x) => x !== id)
            : [...completedAdminTaskIds, id];
        setCompletedAdminTaskIds(next);
        persistAll({ completedAdminTaskIds: next });
    };
    ...
    const adminAddTask = async (data) => {
        const result = await runAdminOp(() => window.FB.addDocTo("adminTasks", data), "টাস্ক");
        // Task Management → Add Task should also land in this admin's own
        // "আমার টাস্ক" list, using the SAME addTask function and the SAME
        // MAX_TASKS=10 cap as every other personal task — addTask already
        // silently no-ops past the cap, so that limit is preserved as-is.
        if (result && result.ok) {
            addTask({
                text: data.title, date: data.dueDate || null, time: data.time || null,
                category: data.category || null, note: data.description || "",
                repeat: data.repeat || null,
                reminderDate: data.reminderDate || null, reminderTime: data.reminderTime || null,
            });
        }
        return result;
    };
    ...
    const adminAddFaq = (data) => runAdminOp(() => window.FB.addDocTo("faq", data), "প্রশ্ন-উত্তর");
    const adminUpdateFaq = (id, patch) => runAdminOp(() => window.FB.updateDocIn("faq", id, patch), "প্রশ্ন-উত্তর");
    const adminDeleteFaq = (id) => runAdminOp(() => window.FB.deleteDocFrom("faq", id), "প্রশ্ন-উত্তর");
    ...
    useEffect(() => {
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [transactions, budget, tasks, specialDays, debts, expenseCats, incomeCats, categoryBudgets, accountOpening, transfers, profileName, familyMembers, bazarItems, taxes, userAccounts, auditLog, readNoticeIds, readDailyMessageIds, dismissedDailyMessageIds, completedAdminTaskIds, user, loaded, autoSync]);
        ...
        return {
            ...
            cloud: cloud
                ? {
                    ...
                    familyMembers: (cloud.familyMembers || []).length,
                    bazarItems: (cloud.bazarItems || []).length,
                    taxes: (cloud.taxes || []).length,
    ...
    /* ---------------- tax management ---------------- */
    const addTax = (tax) => {
        const next = [...taxes, Object.assign(Object.assign({}, tax), { id: uid(), createdAt: Date.now(), paid: false })];
        setTaxes(next);
        persistAll({ taxes: next });
    };
    const updateTax = (id, patch) => {
        const next = taxes.map((t) => (t.id === id ? Object.assign(Object.assign({}, t), patch) : t));
        setTaxes(next);
        persistAll({ taxes: next });
    };
    const deleteTax = (id) => {
    ...
    const toggleTaxPaid = (id) => {
        const next = taxes.map((t) => (t.id === id ? Object.assign(Object.assign({}, t), { paid: !t.paid }) : t));
        setTaxes(next);
        persistAll({ taxes: next });
    };
    ...
    const addTask = (data) => {
        // backward compatible: a plain string still works exactly as
        // before (quick single-line add); an object carries the fuller
        // field set from the new detailed Task form
        const fields = typeof data === "string" ? { text: data } : data;
        const trimmed = (fields.text || "").trim();
        if (!trimmed || tasks.length >= MAX_TASKS)
            return;
        const next = [...tasks, {
                id: uid(), text: trimmed, done: false,
    ...
    const updateTask = (id, patch) => {
        const next = tasks.map((t) => (t.id === id ? Object.assign(Object.assign({}, t), patch) : t));
        setTasks(next);
        persistAll({ tasks: next });
    };
    const toggleTask = (id) => {
    ...
    // reassigns every transaction using fromKey to toKey, then removes
    // fromKey from the category list — used when a category that's already
    // in use needs to go away. Historical transactions keep their amounts
    // and dates; only the category tag changes, so nothing is lost.
    const migrateCategory = (type, fromKey, toKey) => {
        const nextTx = transactions.map((t) => (t.type === type && t.category === fromKey ? Object.assign(Object.assign({}, t), { category: toKey }) : t));
        setTransactions(nextTx);
        if (type === "income") {
            const next = incomeCats.filter((c) => c.key !== fromKey);
            setIncomeCats(next);
            persistAll({ transactions: nextTx, incomeCats: next });
        }
        else {
            const next = expenseCats.filter((c) => c.key !== fromKey);
            setExpenseCats(next);
            persistAll({ transactions: nextTx, expenseCats: next });
        }
    };
            ...
            if (fired.length > 0) {
                ...
                try {
                    if (typeof Notification !== "undefined" && Notification.permission === "granted") {
                        const title = `${dashboardTitle(profileName)} — রিমাইন্ডার`;
                        const body = fired.join(", ");
                        // prefer the service worker's showNotification — more
                        // reliable for an installed PWA (survives the tab
                        // losing focus) than the raw Notification constructor.
                        // Still requires the page/SW to be alive, though —
                        // true notify-while-fully-closed needs a push
                        // server, which this client-only app doesn't have.
    ...
    return (React.createElement(CatCtx.Provider, { value: { expenseCats, incomeCats } },
        React.createElement("div", { style: styles.app },
            ...
            showCalendar && (React.createElement(CalendarModal, { specialDays: specialDays, onSaveSpecialDays: saveSpecialDays, onClose: () => closeMenuPanel(setShowCalendar), onRegisterBackHandler: (fn) => { calendarBackConsumedRef.current = fn; }, adminSpecialDays: adminSpecialDaysCloud })),
            ...
            showSettings && (React.createElement(SettingsModal, { transactions: transactions, budget: budget, specialDays: specialDays, onSaveSpecialDays: saveSpecialDays, onClose: () => closeMenuPanel(setShowSettings), onEditBudget: () => {
                    setShowSettings(false);
                    setShowBudget(true);
                }, onClearAll: () => {
                    clearAll();
                    setShowSettings(false);
                }, accounts: accounts, accountOpening: accountOpening, onSaveAccountOpening: saveAccountOpening, onAddCategory: addCategory, onUpdateCategory: updateCategory, onDeleteCategory: deleteCategory, onMigrateCategory: migrateCategory, pin: pin, onSavePin: savePin, onImportTransactions: (rows) => {
            ...
            showHamburgerMenu && (React.createElement(HamburgerMenu, {
                ...
                onOpenNotifications: () => { menuOriginRef.current = true; setShowHamburgerMenu(false); setTimeout(() => setShowNotificationCenter(true), 0); },
                onOpenSettings: () => { menuOriginRef.current = true; setShowHamburgerMenu(false); setShowSettings(true); },
                onOpenCalendar: () => { menuOriginRef.current = true; setShowHamburgerMenu(false); setShowCalendar(true); },
                onOpenFAQ: () => { menuOriginRef.current = true; setShowHamburgerMenu(false); setTimeout(() => setShowFAQ(true), 0); },
                onOpenTax: () => { menuOriginRef.current = true; setShowHamburgerMenu(false); setTimeout(() => setShowTaxPanel(true), 0); },
            ...
            showAdminPanel && isAdmin(user) && (React.createElement(AdminPanel, {
                onClose: () => closeMenuPanel(setShowAdminPanel),
                notices: notices, dailyMessages: dailyMessages, adminSpecialDaysCloud: adminSpecialDaysCloud, adminTasksCloud: adminTasksCloud, faqCloud: faqCloud,
                ...
                onAddFaq: adminAddFaq, onUpdateFaq: adminUpdateFaq, onDeleteFaq: adminDeleteFaq,
            })),
...
/* ---------------- header ---------------- */
function TaskForm({ initial, onClose, onSave }) {
    const cats = useCategories();
    const [text, setText] = useState((initial === null || initial === void 0 ? void 0 : initial.text) || "");
    const [date, setDate] = useState((initial === null || initial === void 0 ? void 0 : initial.date) || "");
    ...
    return (React.createElement(ModalShell, { onClose: onClose, title: initial ? "টাস্ক সম্পাদনা" : "নতুন টাস্ক" },
        ...
        React.createElement("input", { style: admStyles.input, value: text, onChange: (e) => setText(e.target.value) }),
        ...
        reminderPreset === "custom" && (React.createElement("div", { style: { display: "flex", gap: 8 } },
            React.createElement("input", { style: admStyles.input, type: "date", value: customReminderDate, onChange: (e) => setCustomReminderDate(e.target.value) }),
            React.createElement("input", { style: admStyles.input, type: "time", value: customReminderTime, onChange: (e) => setCustomReminderTime(e.target.value) }))),
...
function Header({ transactions, onSettings, tasks, onAddTask, onUpdateTask, onToggleTask, onDeleteTask, onSetReminder, onOpenCalendar, profileName, onOpenMenu, onOpenNotifications, unreadCount, adminTasksCloud, completedAdminTaskIds, onToggleAdminTaskComplete }) {
    ...
    // completing a task (check mark) marks it done but keeps it in `tasks`
    // (so the existing MAX_TASKS cap and history are unaffected) — it just
    // drops out of the visible active list immediately.
    const activeTasks = tasks.filter((t) => !t.done);
    const activeAdminTasks = (adminTasksCloud || []).filter((t) => t.published !== false && !(completedAdminTaskIds || []).includes(t.id));
            ...
            React.createElement("div", { style: { position: "relative", display: "flex", gap: 8 } },
                ...
                showTasks && (React.createElement(React.Fragment, null,
                    ...
                    React.createElement("div", { style: styles.taskDropdown },
                        ...
                        tasks.length === 0 ? (React.createElement("div", { style: styles.taskEmpty }, "\u098F\u0996\u09A8\u09CB \u0995\u09CB\u09A8\u09CB \u0995\u09BE\u099C \u09AF\u09CB\u0997 \u0995\u09B0\u09BE \u09B9\u09AF\u09BC\u09A8\u09BF\u0964")) : activeTasks.length === 0 ? (React.createElement("div", { style: styles.taskEmpty }, "\u09B8\u09AC \u0995\u09BE\u099C \u09B8\u09AE\u09CD\u09AA\u09A8\u09CD\u09A8 \u09B9\u09AF\u09BC\u09C7\u099B\u09C7\u0964")) : (React.createElement("div", { style: styles.taskList }, activeTasks.map((t) => (React.createElement("div", { key: t.id },
                        ...
                        activeAdminTasks.length > 0 && (React.createElement("div", { style: { marginTop: 12, paddingTop: 10, borderTop: "1px solid var(--hk-border-light)" } },
                            React.createElement("div", { style: { fontSize: 11.5, fontWeight: 700, color: "var(--hk-gold)", marginBottom: 4 } }, "\u2B50 \u0985\u09CD\u09AF\u09BE\u09A1\u09AE\u09BF\u09A8 \u099F\u09BE\u09B8\u09CD\u0995"),
                            React.createElement("div", { style: styles.taskList }, activeAdminTasks.map((t) => (React.createElement("div", { key: t.id, style: styles.taskItem },
                        ...
                        showTaskForm && (React.createElement(TaskForm, { initial: editingTask, onClose: () => setShowTaskForm(false), onSave: (data) => { editingTask ? onUpdateTask(editingTask.id, data) : onAddTask(data); setShowTaskForm(false); } }))))))))));
}
/* ---------------- dashboard ---------------- */
function Dashboard({ transactions, budget, categoryBudgets, debts, accounts, onEditBudget, onOpenTx, onGoDebts, onGoReports, onQuickAdd, onTransfer, onTransferHistory, profileName, taxes, bazarItems }) {
    const cats = useCategories();
    ...
    // unpaid taxes due soon/overdue — same urgency window as debt alerts,
    // shown as its own small stack right under them (upcoming obligations)
    const taxAlerts = useMemo(() => {
        return (taxes || [])
            .map((t) => ({ t, info: taxDueInfo(t) }))
            .filter((x) => x.info && (x.info.key === "overdue" || x.info.key === "today" || x.info.key === "soon"))
            .sort((a, b) => a.info.daysLeft - b.info.daysLeft)
            .slice(0, 3);
    }, [taxes]);
    // this month's family bazar spending (purchased items only — matches
    // how the Family module counts "actual" spend)
    const bazarSpendThisMonth = useMemo(() => (bazarItems || [])
        .filter((it) => it.purchased && monthKeyOf(it.purchaseDate || "") === thisMonthKey)
        .reduce((s, it) => s + computeBazarTotal(it), 0), [bazarItems, thisMonthKey]);
    ...
    return (React.createElement("div", { style: styles.pageLedger },
        ...
        dueAlerts.length > 0 && (React.createElement("div", { style: styles.alertStack }, dueAlerts.map(({ d, info }) => (React.createElement("button", { key: d.id, style: Object.assign(Object.assign({}, styles.alertCard), { borderColor: info.color }), onClick: onGoDebts },
            React.createElement("span", { style: { color: info.color } },
                ...
                " \u2014 ",
                info.label)))))),
        taxAlerts.length > 0 && (React.createElement("div", { style: styles.alertStack }, taxAlerts.map(({ t, info }) => (React.createElement("div", { key: t.id, style: Object.assign(Object.assign({}, styles.alertCard), { borderColor: info.color }) },
            React.createElement("span", { style: { color: info.color } },
                "\u099F\u09CD\u09AF\u09BE\u0995\u09CD\u09B8: ",
                t.name,
                " \u2014 ",
        ...
        bazarSpendThisMonth > 0 && (React.createElement("div", { style: styles.savingsCard },
            React.createElement("div", { style: styles.savingsRow },
                React.createElement("span", { style: styles.statLabel }, "\u098F\u0987 \u09AE\u09BE\u09B8\u09C7 \u09AC\u09BE\u099C\u09BE\u09B0 \u0996\u09B0\u099A (\u09AB\u09CD\u09AF\u09BE\u09AE\u09BF\u09B2\u09BF)"),
                React.createElement("span", { style: { fontFamily: "'JetBrains Mono', monospace", fontWeight: 700 } }, formatTaka(bazarSpendThisMonth))))),
...
function Reports({ transactions, categoryBudgets, budget, onSearch }) {
    const cats = useCategories();
    ...
    return (React.createElement("div", { style: styles.pageLedger },
        React.createElement("div", { style: Object.assign(Object.assign({}, styles.rangeRow), { alignItems: "center" }) },
            RANGE_OPTIONS.map((r) => (React.createElement("button", { key: r.key, onClick: () => setRange(r.key), style: Object.assign(Object.assign({}, styles.rangeChip), (range === r.key ? styles.rangeChipActive : {})) }, r.label))),
            React.createElement("button", { style: Object.assign(Object.assign({}, styles.rangeChip), { marginLeft: "auto", fontWeight: 700 }), onClick: () => onSearch && onSearch(start, end) }, "\uD83D\uDD0D \u0996\u09C1\u0981\u099C\u09C1\u09A8")),
...
function SearchView({ transactions, onOpenTx, seed, familyMembers, userAccounts, debts }) {
    ...
    return (React.createElement("div", { style: styles.pageLedger },
        ...
        React.createElement("div", { style: styles.filterRow },
            React.createElement("select", { style: styles.filterSelect, value: category, onChange: (e) => setCategory(e.target.value) },
                React.createElement("option", { value: "all" }, "\u09B8\u09AC \u0995\u09CD\u09AF\u09BE\u099F\u09C7\u0997\u09B0\u09BF"),
                (type !== "income" ? cats.expenseCats : []).map((c) => React.createElement("option", { key: `e-${c.key}`, value: c.key }, c.label)),
...
function NoticeForm({ initial, onClose, onSave }) {
    ...
    const [err, setErr] = useState("");
    return (React.createElement(ModalShell, { onClose: onClose, fullScreen: true, title: initial ? "নোটিশ সম্পাদনা" : "নতুন নোটিশ" },
...
function DailyMessageForm({ initial, onClose, onSave }) {
    ...
    const [err, setErr] = useState("");
    return (React.createElement(ModalShell, { onClose: onClose, fullScreen: true, title: initial ? "বার্তা সম্পাদনা" : "নতুন দৈনিক বার্তা" },
        React.createElement("label", { style: admStyles.label }, "শিরোনাম *"),
        React.createElement("input", { style: admStyles.input, value: title, onChange: (e) => setTitle(e.target.value) }),
...
function TaskAdminForm({ initial, onClose, onSave }) {
    ...
    // computeTaskReminderFields already used by the personal task form
    // (TaskForm above) — reused as-is, not duplicated, so both task forms
    // offer identical reminder/repeat choices. No amount/price/টাকা field
    // here, matching the personal task form.
    const cats = useCategories();
    const [title, setTitle] = useState((initial === null || initial === void 0 ? void 0 : initial.title) || "");
    const [date, setDate] = useState((initial === null || initial === void 0 ? void 0 : initial.dueDate) || "");
    const [time, setTime] = useState((initial === null || initial === void 0 ? void 0 : initial.time) || "");
    const [category, setCategory] = useState((initial === null || initial === void 0 ? void 0 : initial.category) || "");
    const [note, setNote] = useState((initial === null || initial === void 0 ? void 0 : initial.description) || "");
    const [repeat, setRepeat] = useState((initial === null || initial === void 0 ? void 0 : initial.repeat) || "none");
    const [reminderPreset, setReminderPreset] = useState((initial === null || initial === void 0 ? void 0 : initial.reminderDate) ? "custom" : "none");
    const [customReminderDate, setCustomReminderDate] = useState((initial === null || initial === void 0 ? void 0 : initial.reminderDate) || "");
    const [customReminderTime, setCustomReminderTime] = useState((initial === null || initial === void 0 ? void 0 : initial.reminderTime) || "");
    const [published, setPublished] = useState((initial === null || initial === void 0 ? void 0 : initial.published) !== false);
    const [err, setErr] = useState("");
    return (React.createElement(ModalShell, { onClose: onClose, fullScreen: true, title: initial ? "টাস্ক সম্পাদনা" : "নতুন অ্যাডমিন টাস্ক" },
        React.createElement("label", { style: admStyles.label }, "টাস্কের নাম *"),
        React.createElement("input", { style: admStyles.input, value: title, onChange: (e) => setTitle(e.target.value) }),
        React.createElement("div", { style: { display: "flex", gap: 8 } },
            React.createElement("div", { style: { flex: 1 } },
        ...
        React.createElement("button", { style: admStyles.addBtn, onClick: () => {
                ...
                const { reminderDate, reminderTime } = computeTaskReminderFields(date, time, reminderPreset, customReminderDate, customReminderTime);
                onSave({ title: title.trim(), description: note.trim(), category: category || null, dueDate: date || null, time: time || null, repeat: repeat === "none" ? null : repeat, reminderDate, reminderTime, published });
            } }, "সংরক্ষণ করুন")));
}
function FaqAdminForm({ initial, onClose, onSave }) {
    const [question, setQuestion] = useState((initial === null || initial === void 0 ? void 0 : initial.question) || "");
    const [answer, setAnswer] = useState((initial === null || initial === void 0 ? void 0 : initial.answer) || "");
    const [err, setErr] = useState("");
    return (React.createElement(ModalShell, { onClose: onClose, fullScreen: true, title: initial ? "প্রশ্ন-উত্তর সম্পাদনা" : "নতুন প্রশ্ন-উত্তর" },
        React.createElement("label", { style: admStyles.label }, "প্রশ্ন *"),
        React.createElement("input", { style: admStyles.input, value: question, onChange: (e) => setQuestion(e.target.value) }),
        React.createElement("label", { style: admStyles.label }, "উত্তর *"),
        React.createElement("textarea", { style: Object.assign(Object.assign({}, admStyles.input), { minHeight: 100 }), value: answer, onChange: (e) => setAnswer(e.target.value) }),
        err && React.createElement("div", { style: { color: "var(--hk-danger)", fontSize: 12.5, marginBottom: 8 } }, err),
...
function AdminPanel({ onClose, notices, dailyMessages, adminSpecialDaysCloud, adminTasksCloud, faqCloud, loading, error, onRefresh, onAddNotice, onUpdateNotice, onDeleteNotice, onAddDailyMessage, onUpdateDailyMessage, onDeleteDailyMessage, onAddSpecialDay, onUpdateSpecialDay, onDeleteSpecialDay, onBulkImport, onAddTask, onUpdateTask, onDeleteTask, onAddFaq, onUpdateFaq, onDeleteFaq, }) {
    ...
    useBackOverlay(showTaskForm, () => setShowTaskForm(false));
    useBackOverlay(showFaqForm, () => setShowFaqForm(false));
    ...
    if (loading) {
        ...
        body.push(React.createElement("button", { key: "add", style: admStyles.addBtn, onClick: () => { setEditingFaq(null); setShowFaqForm(true); } }, "+ নতুন প্রশ্ন-উত্তর"));
        if ((faqCloud || []).length === 0)
            body.push(React.createElement(EmptyState, { key: "empty", text: "এখনও কোনো প্রশ্ন-উত্তর নেই।" }));
        (faqCloud || []).forEach((f) => body.push(React.createElement("div", { key: f.id, style: admStyles.card },
            React.createElement("div", { style: admStyles.row },
                React.createElement("div", null,
                    React.createElement("div", { style: { fontWeight: 600 } }, f.question),
                    React.createElement("div", { style: admStyles.muted }, f.answer)),
                rowActions(f, () => { setEditingFaq(f); setShowFaqForm(true); }, onDeleteFaq)))));
    }
    if (showNoticeForm)
    ...
    if (showFaqForm)
        body.push(React.createElement(FaqAdminForm, { key: "fform", initial: editingFaq, onClose: () => setShowFaqForm(false), onSave: (data) => runAndClose(editingFaq ? onUpdateFaq(editingFaq.id, data) : onAddFaq(data), () => setShowFaqForm(false)) }));
    if (showBulk)
    ...
    return React.createElement(ModalShell, { onClose: onClose, fullScreen: true, title: "\u098F\u09A1\u09AE\u09BF\u09A8 \u09AA\u09CD\u09AF\u09BE\u09A8\u09C7\u09B2" },
        React.createElement("div", { style: admStyles.subTabs },
            ...
            React.createElement("button", { style: admStyles.subTabBtn(tab === "faq"), onClick: () => setTab("faq") }, "\u2753 FAQ")),
        ...body);
}
/* ---------------- notification center ---------------- */
function NotificationCenter({ onClose, notices, dailyMessages, tasks, debts, specialDays, adminSpecialDaysCloud, adminTasksCloud, taxes, readNoticeIds, readDailyMessageIds, dismissedDailyMessageIds, onMarkNoticeRead, onMarkDailyMessageRead, onDismissDailyMessage, onMarkAllRead, }) {
    ...
    return React.createElement(ModalShell, { onClose: onClose, fullScreen: true, title: "\u09A8\u09CB\u099F\u09BF\u09AB\u09BF\u0995\u09C7\u09B6\u09A8" },
        React.createElement("div", { style: admStyles.subTabs },
...
/* ---------------- hamburger menu ---------------- */
function HamburgerMenu({ onClose, onOpenNotifications, onOpenSettings, onOpenCalendar, onOpenFAQ, onOpenTax, onOpenAccounts, onOpenFamilyBazar, isAdminUser, onOpenAdmin, unreadCount, }) {
    ...
    return React.createElement("div", { style: { position: "fixed", inset: 0, zIndex: 200, display: "flex" } },
        React.createElement("div", { style: { width: "50%", background: "var(--hk-card)", height: "100%", overflowY: "auto", overscrollBehaviorY: "contain", padding: "20px 18px", boxSizing: "border-box", display: "flex", flexDirection: "column" } },
            ...
            React.createElement("button", { style: menuItemStyle, onClick: onOpenFamilyBazar }, "\uD83D\uDED2 \u09AB\u09CD\u09AF\u09BE\u09AE\u09BF\u09B2\u09BF \u09AC\u09BE\u099C\u09BE\u09B0"),
...
function FAQModal({ onClose, faqCloud }) {
    ...
    // through the Admin Panel's FAQ tab, same pattern as Notice/Daily
    // Message/Special Day/Task
    const faqs = faqCloud || [];
    return React.createElement(ModalShell, { onClose: onClose, fullScreen: true, title: "\u09AA\u09CD\u09B0\u09B6\u09CD\u09A8 \u0993 \u0989\u09A4\u09CD\u09A4\u09B0" }, faqs.length === 0
        ? React.createElement(EmptyState, { text: "এখনও কোনো প্রশ্ন-উত্তর যোগ করা হয়নি।" })
        : faqs.map((f) => React.createElement("div", { key: f.id, style: admStyles.card },
            React.createElement("div", { style: { fontWeight: 600, marginBottom: 4 } }, f.question),
            React.createElement("div", { style: admStyles.muted }, f.answer))));
}
/* ---------------- tax management ---------------- */
...
const FB_ROLES = [
    { key: "admin", label: "এডমিন" }, { key: "member", label: "সদস্য" }, { key: "viewer", label: "ভিউয়ার" },
];
const FB_PERMISSION_KEYS = [
    { key: "monthlyTotal", label: "মাসিক মোট খরচ" },
    { key: "categorySummary", label: "ক্যাটাগরি অনুযায়ী সারাংশ" },
    { key: "purchaseDetails", label: "প্রতিটি কেনাকাটার বিস্তারিত" },
    { key: "priceHistory", label: "পণ্যের দামের ইতিহাস" },
    { key: "locationComparison", label: "এলাকাভিত্তিক দাম তুলনা" },
    { key: "reports", label: "রিপোর্ট" },
];
const FB_DEFAULT_PERMISSIONS = { monthlyTotal: true, categorySummary: true, purchaseDetails: false, priceHistory: true, locationComparison: false, reports: true, memberManagement: false };
const FB_DEFAULT_CATEGORIES = [
    { key: "rice", label: "চাল" }, { key: "lentil", label: "ডাল" }, { key: "oil", label: "তেল" },
    { key: "salt", label: "লবণ" }, { key: "spice", label: "মসলা" }, { key: "vegetable", label: "সবজি" },
    { key: "fish", label: "মাছ" }, { key: "meat", label: "মাংস" }, { key: "egg", label: "ডিম" },
    { key: "milk", label: "দুধ" }, { key: "fruit", label: "ফল" }, { key: "potato", label: "আলু" },
    { key: "onion", label: "পেঁয়াজ" }, { key: "garlic", label: "রসুন" },
    { key: "soap", label: "সাবান" }, { key: "detergent", label: "ডিটারজেন্ট" }, { key: "tissue", label: "টিস্যু" },
    { key: "cleaning", label: "পরিষ্কারের সামগ্রী" }, { key: "other", label: "অন্যান্য" },
];
const FB_UNITS = ["kg", "g", "L", "ml", "pcs", "ডজন", "প্যাকেট"];
...
function FamilyBazarModule({ onClose, user }) {
    ...
    const myMember = members.find((m) => m.uid === (user && user.uid));

    const loadTopLevel = () => {
        if (!user || !window.FB) return;
        setLoading(true);
        Promise.all([window.FB.myFamilies(), window.FB.myIncomingInvitations()])
            .then(([fams, invs]) => {
                setFamilies(fams);
                setIncoming(invs);
                if (!activeFamilyId && fams.length > 0) setActiveFamilyId(fams[0].id);
            })
            .catch((e) => setError(e.message || "লোড করা যায়নি"))
            .finally(() => setLoading(false));
    };

    const loadFamilyData = (familyId) => {
        if (!familyId || !window.FB) return;
        setLoading(true);
        Promise.all([
            window.FB.familyMembers(familyId),
    ...
    useEffect(() => { loadTopLevel(); }, [user]);
    useEffect(() => { if (activeFamilyId) loadFamilyData(activeFamilyId); }, [activeFamilyId]);

    const activeFamily = families.find((f) => f.id === activeFamilyId);

    const handleCreateFamily = (name) => {
        window.FB.createFamily(name).then((id) => {
            setShowCreate(false);
            loadTopLevel();
            setActiveFamilyId(id);
        }).catch((e) => setError(e.message || "তৈরি করা যায়নি"));
    };

    const handleAcceptInvite = (inv) => {
        window.FB.acceptInvitation(inv).then(() => { loadTopLevel(); setActiveFamilyId(inv.familyId); }).catch((e) => setError(e.message));
    };
    const handleRejectInvite = (inv) => {
        window.FB.rejectInvitation(inv.id).then(() => loadTopLevel()).catch((e) => setError(e.message));
    };

    const handleInvite = (payload) => {
        window.FB.sendInvitation(Object.assign({ familyId: activeFamilyId, familyName: activeFamily ? activeFamily.name : "" }, payload))
            .then(() => setShowInvite(false))
            .catch((e) => setError(e.message || "Invite পাঠানো যায়নি"));
    };

    const handleAddPurchase = (purchase) => {
        window.FB.addFamilyPurchase(activeFamilyId, Object.assign({ memberId: user.uid }, purchase))
            .then(() => { setShowAddPurchase(false); loadFamilyData(activeFamilyId); })
            .catch((e) => setError(e.message || "সংরক্ষণ করা যায়নি"));
    };

    const handleDeletePurchase = (id) => {
        window.FB.deleteFamilyPurchase(activeFamilyId, id).then(() => loadFamilyData(activeFamilyId)).catch((e) => setError(e.message));
    };
    ...
    if (!user) {
        return React.createElement(ModalShell, { onClose: onClose, fullScreen: true, title: "🛒 ফ্যামিলি বাজার" },
            React.createElement("div", { style: styles.taskEmpty }, "ফ্যামিলি বাজার ব্যবহার করতে হলে আগে Google দিয়ে Sign In করতে হবে — কারণ এটা একাধিক পরিবারের সদস্যের সাথে ডেটা শেয়ার করে।"),
            React.createElement("button", { style: admStyles.addBtn, onClick: () => window.FB && window.FB.signInGoogle().catch((e) => setError(e.message)) }, "Google দিয়ে Sign In করুন"),
            error && React.createElement("div", { style: { color: "var(--hk-danger)", fontSize: 12.5, marginTop: 10 } }, error));
    }
    ...
    const prevMonthTotal = purchases.filter((p) => monthKeyOf(p.date) === prevMonthKey).reduce((s, p) => s + (p.total || 0), 0);
    const pctChange = prevMonthTotal > 0 ? Math.round(((monthTotal - prevMonthTotal) / prevMonthTotal) * 100) : null;

    const canSeeMember = (m) => m.uid === user.uid || (myMember && myMember.permissions && myMember.permissions.purchaseDetails);
    const memberTotals = members.map((m) => ({
        member: m,
        total: monthPurchases.filter((p) => p.memberId === m.uid).reduce((s, p) => s + (p.total || 0), 0),
        visible: m.uid === user.uid || (myMember && myMember.permissions && myMember.permissions.monthlyTotal),
    }));

    const spentPct = budget && budget.monthlyAmount ? Math.min(100, Math.round((monthTotal / budget.monthlyAmount) * 100)) : null;

    return React.createElement(ModalShell, { onClose: onClose, fullScreen: true, title: "🛒 ফ্যামিলি বাজার" },
        error && React.createElement("div", { style: { color: "var(--hk-danger)", fontSize: 12.5, marginBottom: 8 } }, error),
        incoming.length > 0 && React.createElement("div", { style: { marginBottom: 14, padding: 10, border: "1px solid var(--hk-gold)", borderRadius: 10 } },
            React.createElement("div", { style: { fontWeight: 700, marginBottom: 6 } }, "📩 নতুন Invitation"),
            incoming.map((inv) => React.createElement("div", { key: inv.id, style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 } },
                React.createElement("span", { style: { fontSize: 13.5 } }, inv.familyName, " — ", inv.invitedByName || "একজন সদস্য", " আপনাকে যোগ করতে চান"),
                React.createElement("span", { style: { display: "flex", gap: 6 } },
                    React.createElement("button", { style: admStyles.addBtn, onClick: () => handleAcceptInvite(inv) }, "গ্রহণ"),
                    React.createElement("button", { style: styles.taskDelete, onClick: () => handleRejectInvite(inv) }, "প্রত্যাখ্যান"))))),

        families.length === 0 && !showCreate && (
            React.createElement("div", { style: { textAlign: "center", padding: "30px 10px" } },
                React.createElement("div", { style: styles.taskEmpty }, "আপনার এখনো কোনো পরিবার নেই। একটা তৈরি করুন অথবা কারো পাঠানো invitation গ্রহণ করুন।"),
                React.createElement("button", { style: admStyles.addBtn, onClick: () => setShowCreate(true) }, "+ পরিবার তৈরি করুন"))),

        showCreate && React.createElement(FBCreateFamilyForm, { onCancel: () => setShowCreate(false), onCreate: handleCreateFamily }),

        activeFamily && !showCreate && React.createElement(React.Fragment, null,
            React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 } },
                React.createElement("button", { onClick: () => setShowSwitcher((v) => !v), style: { background: "none", border: "1px solid var(--hk-border-med)", borderRadius: 8, padding: "6px 10px", fontWeight: 700 } }, activeFamily.name, " ▾"),
                React.createElement("button", { style: { background: "none", border: "none", color: "var(--hk-text-muted)", fontSize: 12.5, textDecoration: "underline" }, onClick: () => setShowCreate(true) }, "+ নতুন পরিবার")),
            ...
            tab === "bazar" && React.createElement(FBBazarTab, {
...
function FBCreateFamilyForm({ onCancel, onCreate }) {
    ...
    return React.createElement("div", { style: { marginBottom: 16 } },
        React.createElement("label", { style: admStyles.label }, "পরিবারের নাম *"),
        React.createElement("input", { style: admStyles.input, value: name, onChange: (e) => setName(e.target.value), placeholder: "যেমন: হোসেন পরিবার" }),
        React.createElement("div", { style: { display: "flex", gap: 8, marginTop: 8 } },
            React.createElement("button", { style: admStyles.addBtn, onClick: () => name.trim() && onCreate(name.trim()) }, "তৈরি করুন"),
            React.createElement("button", { style: styles.taskDelete, onClick: onCancel }, "বাতিল")));
}

function FBHomeTab({ monthTotal, pctChange, memberTotals, budget, spentPct, onAddPurchase }) {
    return React.createElement("div", null,
        React.createElement("div", { style: Object.assign(Object.assign({}, styles.calDayDetailCard), { textAlign: "center", marginBottom: 14 }) },
            React.createElement("div", { style: { fontSize: 12.5, color: "var(--hk-text-muted)" } }, "এই মাসে পরিবারে বাজার খরচ"),
            React.createElement("div", { style: { fontSize: 28, fontWeight: 800, margin: "4px 0" } }, formatTaka(monthTotal)),
            pctChange !== null && React.createElement("div", { style: { fontSize: 12.5, color: pctChange >= 0 ? "var(--hk-danger)" : "var(--hk-success)" } }, pctChange >= 0 ? `+${pctChange}%` : `${pctChange}%`, " গত মাসের তুলনায়")),
        budget && budget.monthlyAmount && React.createElement("div", { style: { marginBottom: 14 } },
            React.createElement("div", { style: { display: "flex", justifyContent: "space-between", fontSize: 12.5, marginBottom: 4 } },
                React.createElement("span", null, "বাজেট ব্যবহার হয়েছে"),
                React.createElement("span", null, spentPct, "%")),
            React.createElement("div", { style: { height: 8, borderRadius: 999, background: "var(--hk-border-light)", overflow: "hidden" } },
                React.createElement("div", { style: { height: "100%", width: `${spentPct}%`, background: spentPct >= 100 ? "var(--hk-danger)" : spentPct >= 80 ? "#E8C547" : "var(--hk-success-mid)" } }))),
        React.createElement("div", { style: styles.formLabel }, "কে কত খরচ করেছে"),
        memberTotals.map(({ member, total, visible }) => React.createElement("div", { key: member.uid, style: styles.taskItem },
            React.createElement("span", { style: styles.taskText }, member.name || member.email),
            React.createElement("span", { style: { fontWeight: 600 } }, visible ? formatTaka(total) : "গোপন"))),
        React.createElement("button", { style: Object.assign(Object.assign({}, admStyles.addBtn), { marginTop: 14 }), onClick: onAddPurchase }, "+ বাজার যোগ করুন"));
}

function FBBazarTab({ purchases, members, myUid, onAdd, onDelete }) {
    const nameOf = (uid) => { const m = members.find((x) => x.uid === uid); return m ? (m.name || m.email) : uid; };
    return React.createElement("div", null,
        React.createElement("button", { style: Object.assign(Object.assign({}, admStyles.addBtn), { marginBottom: 14 }), onClick: onAdd }, "+ বাজার যোগ করুন"),
        purchases.length === 0 ? React.createElement("div", { style: styles.taskEmpty }, "আজকের বাজারের হিসাব এখনো যোগ করা হয়নি।")
            : purchases.map((p) => React.createElement("div", { key: p.id, style: Object.assign(Object.assign({}, styles.calSpecialEditRow), { alignItems: "flex-start", flexDirection: "column" }) },
                React.createElement("div", { style: { display: "flex", justifyContent: "space-between", width: "100%" } },
                    React.createElement("span", { style: { fontWeight: 600 } }, formatDateBn(p.date).full, " — ", nameOf(p.memberId)),
                    React.createElement("span", { style: { fontWeight: 700 } }, formatTaka(p.total))),
                (p.market || p.location) && React.createElement("div", { style: { fontSize: 12, color: "var(--hk-text-muted)" } }, [p.market, p.location].filter(Boolean).join(" · ")),
                (p.items || []).map((it, idx) => React.createElement("div", { key: idx, style: { fontSize: 12.5, marginTop: 2 } }, it.productName, " — ", it.quantity, it.unit, " × ", formatTaka(it.unitPrice), " = ", formatTaka(it.total))),
...
function FBBudgetTab({ budget, monthTotal, spentPct, canEdit, onSave }) {
    const [val, setVal] = useState(budget && budget.monthlyAmount ? String(budget.monthlyAmount) : "");
    return React.createElement("div", null,
        canEdit && React.createElement(React.Fragment, null,
            React.createElement("label", { style: admStyles.label }, "মাসিক বাজেট"),
            React.createElement("div", { style: { display: "flex", gap: 8 } },
                React.createElement("input", { style: admStyles.input, type: "number", value: val, onChange: (e) => setVal(e.target.value) }),
                React.createElement("button", { style: admStyles.addBtn, onClick: () => { const n = parseFloat(val); if (n > 0) onSave(n); } }, "সংরক্ষণ"))),
        budget && budget.monthlyAmount && React.createElement("div", { style: { marginTop: 14 } },
            React.createElement("div", null, "বাজেট: ", formatTaka(budget.monthlyAmount)),
            React.createElement("div", null, "খরচ হয়েছে: ", formatTaka(monthTotal)),
            React.createElement("div", null, "বাকি: ", formatTaka(Math.max(0, budget.monthlyAmount - monthTotal))),
            spentPct >= 80 && React.createElement("div", { style: { color: spentPct >= 100 ? "var(--hk-danger)" : "#E8A93B", marginTop: 6, fontWeight: 600 } }, spentPct >= 100 ? "⚠️ বাজেট শেষ হয়ে গেছে!" : spentPct >= 90 ? "⚠️ বাজেটের ৯০% ব্যবহার হয়ে গেছে" : "⚠️ বাজেটের ৮০% ব্যবহার হয়ে গেছে")));
}

function FBAddPurchaseForm({ products, onClose, onSave }) {
    const [date, setDate] = useState(todayStr());
    const [market, setMarket] = useState("");
    const [location, setLocation] = useState("");
    const [note, setNote] = useState("");
    const [items, setItems] = useState([{ productName: "", category: "", quantity: "", unit: "kg", unitPrice: "" }]);
    const [err, setErr] = useState("");

    const updateItem = (idx, patch) => setItems(items.map((it, i) => (i === idx ? Object.assign(Object.assign({}, it), patch) : it)));
    const addRow = () => setItems([...items, { productName: "", category: "", quantity: "", unit: "kg", unitPrice: "" }]);
    const removeRow = (idx) => setItems(items.filter((_, i) => i !== idx));
    const grandTotal = items.reduce((s, it) => s + (parseFloat(it.quantity) || 0) * (parseFloat(it.unitPrice) || 0), 0);

    return React.createElement(ModalShell, { title: "নতুন বাজার", fullScreen: true, onClose: onClose },
        React.createElement("div", { style: { display: "flex", gap: 8 } },
            React.createElement("div", { style: { flex: 1 } },
                React.createElement("label", { style: admStyles.label }, "তারিখ"),
                React.createElement("input", { style: admStyles.input, type: "date", value: date, onChange: (e) => setDate(e.target.value) })),
            React.createElement("div", { style: { flex: 1 } },
                React.createElement("label", { style: admStyles.label }, "বাজার/দোকান"),
                React.createElement("input", { style: admStyles.input, value: market, onChange: (e) => setMarket(e.target.value) }))),
        React.createElement("label", { style: admStyles.label }, "এলাকা"),
        React.createElement("input", { style: admStyles.input, value: location, onChange: (e) => setLocation(e.target.value) }),
        React.createElement("div", { style: { marginTop: 10, marginBottom: 4, fontWeight: 700 } }, "পণ্য"),
        items.map((it, idx) => React.createElement("div", { key: idx, style: { border: "1px solid var(--hk-border-light)", borderRadius: 8, padding: 8, marginBottom: 8 } },
            React.createElement("input", { style: admStyles.input, placeholder: "পণ্যের নাম", value: it.productName, onChange: (e) => updateItem(idx, { productName: e.target.value }) }),
            React.createElement("div", { style: { display: "flex", gap: 6, marginTop: 6 } },
                React.createElement("input", { style: Object.assign(Object.assign({}, admStyles.input), { flex: 1 }), type: "number", placeholder: "পরিমাণ", value: it.quantity, onChange: (e) => updateItem(idx, { quantity: e.target.value }) }),
                React.createElement("select", { style: Object.assign(Object.assign({}, admStyles.input), { flex: 1 }), value: it.unit, onChange: (e) => updateItem(idx, { unit: e.target.value }) }, FB_UNITS.map((u) => React.createElement("option", { key: u, value: u }, u))),
                React.createElement("input", { style: Object.assign(Object.assign({}, admStyles.input), { flex: 1 }), type: "number", placeholder: "একক দাম", value: it.unitPrice, onChange: (e) => updateItem(idx, { unitPrice: e.target.value }) })),
            React.createElement("div", { style: { display: "flex", justifyContent: "space-between", marginTop: 6, fontSize: 12.5 } },
                React.createElement("span", null, "মোট: ", formatTaka((parseFloat(it.quantity) || 0) * (parseFloat(it.unitPrice) || 0))),
                items.length > 1 && React.createElement("button", { style: styles.taskDelete, onClick: () => removeRow(idx) }, "✕")))),
        React.createElement("button", { style: { background: "none", border: "1px dashed var(--hk-border-med)", borderRadius: 8, padding: "8px 12px", width: "100%", marginBottom: 10 }, onClick: addRow }, "+ আরেকটা পণ্য যোগ করুন"),
        React.createElement("label", { style: admStyles.label }, "নোট"),
        React.createElement("input", { style: admStyles.input, value: note, onChange: (e) => setNote(e.target.value) }),
        React.createElement("div", { style: { fontWeight: 700, fontSize: 16, margin: "10px 0" } }, "সর্বমোট: ", formatTaka(grandTotal)),
        err && React.createElement("div", { style: { color: "var(--hk-danger)", fontSize: 12.5, marginBottom: 8 } }, err),
        React.createElement("button", { style: admStyles.addBtn, onClick: () => {
                const cleanItems = items.filter((it) => it.productName.trim() && parseFloat(it.quantity) > 0);
                if (cleanItems.length === 0) { setErr("অন্তত একটা পণ্য দিন"); return; }
                const finalItems = cleanItems.map((it) => ({ productName: it.productName.trim(), category: it.category || null, quantity: parseFloat(it.quantity), unit: it.unit, unitPrice: parseFloat(it.unitPrice) || 0, total: (parseFloat(it.quantity) || 0) * (parseFloat(it.unitPrice) || 0) }));
                onSave({ date, market: market.trim() || null, location: location.trim() || null, note: note.trim(), items: finalItems, total: finalItems.reduce((s, it) => s + it.total, 0) });
            } }, "সংরক্ষণ করুন"));
}

function FBInviteForm({ onClose, onSave }) {
    const [email, setEmail] = useState("");
    const [relation, setRelation] = useState("other");
    const [role, setRole] = useState("member");
    const [permissions, setPermissions] = useState(FB_DEFAULT_PERMISSIONS);
    const [err, setErr] = useState("");
    const togglePerm = (key) => setPermissions(Object.assign(Object.assign({}, permissions), { [key]: !permissions[key] }));
    return React.createElement(ModalShell, { title: "সদস্যকে Invite করুন", fullScreen: true, onClose: onClose },
        React.createElement("label", { style: admStyles.label }, "Gmail *"),
        React.createElement("input", { style: admStyles.input, type: "email", value: email, onChange: (e) => setEmail(e.target.value), placeholder: "example@gmail.com" }),
        React.createElement("label", { style: admStyles.label }, "সম্পর্ক"),
        React.createElement("select", { style: admStyles.input, value: relation, onChange: (e) => setRelation(e.target.value) }, FB_RELATIONS.map((r) => React.createElement("option", { key: r.key, value: r.key }, r.label))),
        React.createElement("label", { style: admStyles.label }, "রোল"),
        React.createElement("select", { style: admStyles.input, value: role, onChange: (e) => setRole(e.target.value) }, FB_ROLES.map((r) => React.createElement("option", { key: r.key, value: r.key }, r.label))),
        React.createElement("div", { style: admStyles.label }, "পারমিশন"),
        FB_PERMISSION_KEYS.map((p) => React.createElement("label", { key: p.key, style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 6, fontSize: 13.5 } },
            React.createElement("input", { type: "checkbox", checked: !!permissions[p.key], onChange: () => togglePerm(p.key) }),
            p.label)),
        err && React.createElement("div", { style: { color: "var(--hk-danger)", fontSize: 12.5, margin: "8px 0" } }, err),
        React.createElement("button", { style: admStyles.addBtn, onClick: () => {
                if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())) { setErr("সঠিক Gmail দিন"); return; }
                onSave({ invitedEmail: email.trim(), relation, role, permissions });
            } }, "Invitation পাঠান"));
}
...
function TaxPanel({ onClose, taxes, onAdd, onUpdate, onDelete, onTogglePaid }) {
    ...
    return React.createElement(ModalShell, { onClose: onClose, fullScreen: true, title: "\u099F\u09CD\u09AF\u09BE\u0995\u09CD\u09B8 \u09AC\u09CD\u09AF\u09AC\u09B8\u09CD\u09A5\u09BE\u09AA\u09A8\u09BE" }, ...body);
}
...
function AccountForm({ initial, onClose, onSave }) {
    ...
    const [err, setErr] = useState("");
    return (React.createElement(ModalShell, { onClose: onClose, fullScreen: true, title: initial ? "\u0985\u09CD\u09AF\u09BE\u0995\u09BE\u0989\u09A8\u09CD\u099F \u09B8\u09AE\u09CD\u09AA\u09BE\u09A6\u09A8\u09BE" : "\u09A8\u09A4\u09C1\u09A8 \u0985\u09CD\u09AF\u09BE\u0995\u09BE\u0989\u09A8\u09CD\u099F" },
...
function AccountPanel({ onClose, dynamicAccountBalances, onAdd, onUpdate, onDelete, onToggleActive, onViewTransactions }) {
    ...
    return React.createElement(ModalShell, { onClose: onClose, fullScreen: true, title: "\u0985\u09CD\u09AF\u09BE\u0995\u09BE\u0989\u09A8\u09CD\u099F \u09AC\u09CD\u09AF\u09AC\u09B8\u09CD\u09A5\u09BE\u09AA\u09A8\u09BE" }, ...body);
}
/* ---------------- calendar modal ---------------- */
...
function CalendarModal({ specialDays, onSaveSpecialDays, onClose, onRegisterBackHandler, adminSpecialDays }) {
    ...
    const [selectedDate, setSelectedDate] = useState(null); // dateStr or null — set by tapping a DATE CELL in the grid; shows every special day that falls on that date
    const [selectedEntry, setSelectedEntry] = useState(null); // { dateStr, title, isAdmin } or null — set by tapping ONE row in the below-calendar list; shows just that single entry, never the whole date's group
    ...
    // a visible list instead of requiring a tap first.
    const monthSpecialEntries = useMemo(() => {
        const y2 = viewDate.getFullYear();
        const m2 = viewDate.getMonth();
        const daysInM = new Date(y2, m2 + 1, 0).getDate();
        const entries = [];
        for (let d = 1; d <= daysInM; d++) {
            const dateStr = todayStr(new Date(y2, m2, d));
            (specialDays[dateStr] || []).forEach((label) => entries.push({ dateStr, day: d, title: label, isAdmin: false }));
            (adminByDate[dateStr] || []).forEach((s) => entries.push({ dateStr, day: d, title: s.title, isAdmin: true }));
        }
        return entries;
    }, [viewDate, specialDays, adminByDate]);
    ...
    }, [selectedDate, selectedEntry, onRegisterBackHandler]);
    ...
    if (selectedEntry) {
        const bn = formatDateBn(selectedEntry.dateStr);
        return (React.createElement(ModalShell, { title: "\u09AC\u09BF\u09B6\u09C7\u09B7 \u09A6\u09BF\u09A8", fullScreen: true, onClose: () => setSelectedEntry(null) },
            React.createElement("button", { style: styles.calBackBtn, onClick: () => setSelectedEntry(null) }, "\u2039 \u0995\u09CD\u09AF\u09BE\u09B2\u09C7\u09A8\u09CD\u09A1\u09BE\u09B0\u09C7 \u09AB\u09BF\u09B0\u09C1\u09A8"),
            React.createElement("div", { style: styles.calDayDetailCard },
                React.createElement("div", { style: styles.calDayDetailDate },
                    bn.weekday,
                    ", ",
                    bn.full),
                React.createElement("div", { style: { marginTop: 10, fontSize: 17, fontWeight: 700, display: "flex", alignItems: "center", gap: 8 } },
                    React.createElement("i", { style: Object.assign(Object.assign({}, styles.legendDot), { background: selectedEntry.isAdmin ? "#E8C547" : "var(--hk-gold)" }) }),
                    selectedEntry.title))));
    }
    if (selectedDate) {
        ...
        return (React.createElement(ModalShell, { title: "\u09AC\u09BF\u09B6\u09C7\u09B7 \u09A6\u09BF\u09A8", fullScreen: true, onClose: () => setSelectedDate(null) },
    ...
    return (React.createElement(ModalShell, { title: "\u0995\u09CD\u09AF\u09BE\u09B2\u09C7\u09A8\u09CD\u09A1\u09BE\u09B0", fullScreen: true, onClose: onClose },
        ...
        React.createElement("div", { style: { margin: "12px 0" } },
            ...
            monthSpecialEntries.length === 0
                ...
                : React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 6 } }, monthSpecialEntries.map((e, i) => React.createElement("button", { key: i, onClick: () => setSelectedEntry(e), style: Object.assign(Object.assign({}, styles.calSpecialEditRow), { width: "100%", textAlign: "left", background: "var(--hk-card)", border: "none", cursor: "pointer" }) },
                    React.createElement("span", { style: { display: "flex", alignItems: "center", gap: 8 } },
                        ...
                        e.title),
                    React.createElement("span", { style: { fontSize: 11.5, color: "var(--hk-text-muted)" } }, toBnDigits(e.day)))))),
...
// same due-date urgency shape as debtDueInfo, for unpaid taxes
function taxDueInfo(t) {
    if (t.paid || !t.dueDate)
        return null;
    const today = todayStr();
    const diffDays = Math.round((new Date(t.dueDate + "T00:00:00") - new Date(today + "T00:00:00")) / 86400000);
    if (diffDays < 0)
        return { key: "overdue", label: `মেয়াদোত্তীর্ণ · ${toBnDigits(Math.abs(diffDays))} দিন পার`, color: "var(--hk-danger)", daysLeft: diffDays };
    if (diffDays === 0)
        return { key: "today", label: "আজই শেষ দিন", color: "var(--hk-gold)", daysLeft: 0 };
...
function TransactionForm({ initial, presetType, onClose, onSave, onDelete, onCheckBalance, userAccounts }) {
    ...
    // (and selected) rather than silently disappearing
    const cats = useMemo(() => {
        const all = type === "income" ? catCtx.incomeCats : catCtx.expenseCats;
        return all.filter((c) => !c.archived || c.key === category);
    }, [type, catCtx, category]);
    useEffect(() => {
...
function CategoryManager({ type, list, transactions, onAdd, onUpdate, onDelete, onMigrate }) {
    const [newLabel, setNewLabel] = useState("");
    const [newIcon, setNewIcon] = useState(CAT_ICON_CHOICES[0]);
    const [editKey, setEditKey] = useState(null);
    const [editLabel, setEditLabel] = useState("");
    const [editIcon, setEditIcon] = useState("");
    const [safetyFor, setSafetyFor] = useState(null); // category key currently showing the delete-safety panel
    ...
    // how many transactions currently use this category — the basis for
    // whether deleting needs the safety panel at all
    const usageCount = (key) => (transactions || []).filter((t) => t.type === type && t.category === key).length;
    const requestDelete = (c) => {
        if (usageCount(c.key) === 0) {
            onDelete(c.key); // nothing references it — safe to remove outright
            return;
        }
        setSafetyFor(c.key);
        setMigrateTarget("");
    ...
    return (React.createElement("div", null,
        list.map((c) => {
            if (editKey === c.key) {
                return React.createElement("div", { key: c.key, style: styles.catEditRow },
                    ...
                    React.createElement("button", { style: styles.taskDelete, onClick: () => setEditKey(null) }, "\u2715"));
            }
            if (safetyFor === c.key) {
                const otherCats = list.filter((o) => o.key !== c.key);
                return React.createElement("div", { key: c.key, style: Object.assign(Object.assign({}, styles.catManageRow), { flexDirection: "column", alignItems: "stretch", gap: 8 }) },
                    React.createElement("div", { style: { fontSize: 12.5 } }, `"${c.label}" ক্যাটেগরিতে ${toBnDigits(usageCount(c.key))}টি লেনদেন আছে। মুছে ফেললে সেগুলো অগোছালো হয়ে যেতে পারে — নিচের একটি নিরাপদ উপায় বেছে নিন:`),
                    React.createElement("div", { style: { display: "flex", gap: 6, flexWrap: "wrap" } },
                        React.createElement("button", { style: styles.catManageBtn, onClick: () => { setSafetyFor(null); startEdit(c); } }, "\u09A8\u09BE\u09AE \u09AA\u09B0\u09BF\u09AC\u09B0\u09CD\u09A4\u09A8 (Rename)"),
                        React.createElement("button", { style: styles.catManageBtn, onClick: () => { onUpdate(c.key, { archived: true }); setSafetyFor(null); } }, "\u0986\u09B0\u09CD\u0995\u09BE\u0987\u09AD (Archive)"),
                        React.createElement("button", { style: styles.catManageBtn, onClick: () => setSafetyFor(`migrate-${c.key}`) }, "\u09B8\u09B0\u09BF\u09AF\u09BC\u09C7 \u09A8\u09BF\u09A8 (Migrate)"),
                        React.createElement("button", { style: styles.taskDelete, onClick: () => setSafetyFor(null) }, "\u09AC\u09BE\u09A4\u09BF\u09B2 (Cancel)")));
            }
            if (safetyFor === `migrate-${c.key}`) {
                const otherCats = list.filter((o) => o.key !== c.key);
                return React.createElement("div", { key: c.key, style: Object.assign(Object.assign({}, styles.catManageRow), { flexDirection: "column", alignItems: "stretch", gap: 8 }) },
                    React.createElement("div", { style: { fontSize: 12.5 } }, `"${c.label}"-এর ${toBnDigits(usageCount(c.key))}টি লেনদেন কোন ক্যাটেগরিতে যাবে?`),
                    React.createElement("select", { style: styles.catIconSelect, value: migrateTarget, onChange: (e) => setMigrateTarget(e.target.value) },
                        React.createElement("option", { value: "" }, "\u2014 \u09A8\u09BF\u09B0\u09CD\u09AC\u09BE\u099A\u09A8 \u0995\u09B0\u09C1\u09A8 \u2014"),
                        otherCats.map((o) => React.createElement("option", { key: o.key, value: o.key }, o.label))),
                    React.createElement("div", { style: { display: "flex", gap: 6 } },
                        React.createElement("button", { style: styles.taskAddBtn, disabled: !migrateTarget, onClick: () => { onMigrate(c.key, migrateTarget); setSafetyFor(null); } }, "\u09A8\u09BF\u09B6\u09CD\u099A\u09BF\u09A4 \u2014 \u09B8\u09B0\u09BF\u09AF\u09BC\u09C7 \u09A8\u09BF\u09A8"),
                        React.createElement("button", { style: styles.taskDelete, onClick: () => setSafetyFor(c.key) }, "\u09AB\u09BF\u09B0\u09C7 \u09AF\u09BE\u09A8")));
            }
            return React.createElement("div", { key: c.key, style: styles.catManageRow },
                React.createElement("span", { style: { flex: 1, opacity: c.archived ? 0.55 : 1 } },
                    React.createElement(CategoryIcon, { catKey: c.key, emoji: c.icon, size: 15 }),
                    " ",
                    c.label,
                    c.archived && React.createElement("span", { style: { fontSize: 10.5, marginLeft: 6 } }, "(\u0986\u09B0\u09CD\u0995\u09BE\u0987\u09AD\u09A1)")),
                c.archived
                    ? React.createElement("button", { style: styles.catManageBtn, onClick: () => onUpdate(c.key, { archived: false }) }, "\u09AB\u09BF\u09B0\u09BF\u09AF\u09BC\u09C7 \u0986\u09A8\u09C1\u09A8")
                    : React.createElement("button", { style: styles.catManageBtn, onClick: () => startEdit(c) }, React.createElement(Icon, { name: "edit", size: 14 })),
                React.createElement("button", { style: styles.catManageBtn, onClick: () => requestDelete(c) }, React.createElement(Icon, { name: "delete", size: 14 })));
        }),
...
function SettingsModal({ transactions, budget, specialDays, onSaveSpecialDays, onClose, onEditBudget, onClearAll, accounts, accountOpening, onSaveAccountOpening, onAddCategory, onUpdateCategory, onDeleteCategory, onMigrateCategory, pin, onSavePin, onImportTransactions, theme, onSaveTheme, user, syncStatus, lastSyncedAt, onShowLogin, onLogout, onManualSync, onRestoreFromCloud, onPreviewCloudVsLocal, isOnline, pendingChanges, onExportJSON, onImportJSON, profileName, onSaveProfileName, autoSync, onSaveAutoSync, notificationPermission, onRequestNotificationPermission, auditLog, }) {
    ...
    return (React.createElement(ModalShell, { onClose: onClose, fullScreen: true, title: "\u09B8\u09C7\u099F\u09BF\u0982\u09B8" },
            ...
            showAccountSection && (React.createElement("div", { style: styles.accountCard },
                ...
                React.createElement("div", { style: styles.syncStatusRow },
                    ...
                    pendingChanges > 0 && React.createElement("span", { style: styles.pendingBadge },
                        ...
                        " \u09AA\u09C7\u09A8\u09CD\u09A1\u09BF\u0982")),
                lastSyncedAt && (React.createElement("div", { style: Object.assign(Object.assign({}, styles.syncStatusRow), { marginTop: -6, opacity: 0.75, fontSize: 11.5 }) }, `🕒 সর্বশেষ সিঙ্ক: ${formatSyncTime(lastSyncedAt)}`)),
                React.createElement("div", { style: styles.formActions },
                ...
                restorePreview && restorePreview !== "loading" && restorePreview !== "error" && (React.createElement("div", { style: styles.confirmBox },
                    ...
                    React.createElement("div", { style: styles.restoreCompareRow },
                        React.createElement("span", null, "\u09AA\u09B0\u09BF\u09AC\u09BE\u09B0\u09C7\u09B0 \u09B8\u09A6\u09B8\u09CD\u09AF"),
                        React.createElement("span", null, toBnDigits(restorePreview.local.familyMembers)),
                        React.createElement("span", null, restorePreview.cloud ? toBnDigits(restorePreview.cloud.familyMembers) : "—")),
                    React.createElement("div", { style: styles.restoreCompareRow },
                        React.createElement("span", null, "\u09AC\u09BE\u099C\u09BE\u09B0 \u0986\u0987\u099F\u09C7\u09AE"),
                        React.createElement("span", null, toBnDigits(restorePreview.local.bazarItems)),
                        React.createElement("span", null, restorePreview.cloud ? toBnDigits(restorePreview.cloud.bazarItems) : "—")),
                    React.createElement("div", { style: styles.restoreCompareRow },
                        React.createElement("span", null, "\u099F\u09CD\u09AF\u09BE\u0995\u09CD\u09B8"),
                        React.createElement("span", null, toBnDigits(restorePreview.local.taxes)),
                        React.createElement("span", null, restorePreview.cloud ? toBnDigits(restorePreview.cloud.taxes) : "—")),
        ...
        React.createElement(SettingsSection, { title: "\u0996\u09BE\u09A4 \u09AC\u09CD\u09AF\u09AC\u09B8\u09CD\u09A5\u09BE\u09AA\u09A8\u09BE" },
            ...
            showCats === "expense" && (React.createElement("div", { style: { marginBottom: 10 } },
                React.createElement(CategoryManager, { type: "expense", list: cats.expenseCats, transactions: transactions, onAdd: (label, icon) => onAddCategory("expense", label, icon), onUpdate: (key, patch) => onUpdateCategory("expense", key, patch), onDelete: (key) => onDeleteCategory("expense", key), onMigrate: (fromKey, toKey) => onMigrateCategory("expense", fromKey, toKey) }))),
            ...
            showCats === "income" && (React.createElement("div", { style: { marginBottom: 10 } },
                React.createElement(CategoryManager, { type: "income", list: cats.incomeCats, transactions: transactions, onAdd: (label, icon) => onAddCategory("income", label, icon), onUpdate: (key, patch) => onUpdateCategory("income", key, patch), onDelete: (key) => onDeleteCategory("income", key), onMigrate: (fromKey, toKey) => onMigrateCategory("income", fromKey, toKey) })))),
        ...
        React.createElement(SettingsSection, { title: "\u09AC\u09CD\u09AF\u09BE\u0995\u0986\u09AA" },
            ...
            showImport && (React.createElement("div", null,
                ...
                React.createElement("div", { style: { height: 10 } })))),
        React.createElement(SettingsSection, { title: "\uD83D\uDD14 \u09A8\u09CB\u099F\u09BF\u09AB\u09BF\u0995\u09C7\u09B6\u09A8" },
            React.createElement("div", { style: styles.settingsRow },
                React.createElement("span", null, "\u09AC\u09CD\u09B0\u09BE\u0989\u099C\u09BE\u09B0 \u09A8\u09CB\u099F\u09BF\u09AB\u09BF\u0995\u09C7\u09B6\u09A8"),
                React.createElement("span", { style: styles.settingsRowValue }, notificationPermission === "granted"
                    ? "✅ অনুমোদিত"
...
/* ---------------- modal shell ---------------- */
// shared counter so nested/stacked modals don't fight over restoring the
// background scroll: body scroll only comes back once the LAST modal
// unmounts, not the first one that happens to close
...
function ModalShell({ title, onClose, children, fullScreen }) {
    // two complementary fixes, both reusable across every panel that uses
    // this one shared shell: (1) overscroll-behavior on the scrollable
    // sheet stops scroll-chaining — once the sheet hits its own scroll
    // boundary, the gesture does NOT bubble to whatever is behind it; (2)
    // locking document.body while any modal is open is a belt-and-suspenders
    // fallback for browsers/cases where (1) alone isn't enough (e.g.
    // touches that start outside the sheet, on the overlay backdrop).
    useBackgroundScrollLock();
    // fullScreen is opt-in per usage — it swaps the default "sheet dropped
    // from the top, dashboard peeking out below/around it" look for a
    // full-page one (no gap, no rounded corners, opaque background).
    // Existing callers that don't pass it are completely unaffected.
    return (React.createElement("div", { style: fullScreen ? styles.modalOverlayFull : styles.modalOverlay, onClick: onClose },
        React.createElement("div", { style: fullScreen ? styles.modalSheetFull : styles.modalSheet, onClick: (e) => e.stopPropagation() },
...
const styles = {
    ...
    // fullscreen variants — used by Calendar, Notification Center, the
    // 3-dot Menu and every panel/step opened from it (Admin Panel, Tax,
    // Accounts, FAQ, Settings, and their nested add/edit forms). Same
    // ModalShell, same header/close-button/back-navigation wiring — only
    // the sizing changes, so nothing about how these panels work is
    // touched, only how much of the screen they occupy.
    modalOverlayFull: {
        position: "fixed",
        inset: 0,
        background: "var(--hk-surface-soft)",
